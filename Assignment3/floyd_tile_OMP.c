...
int main (int argc, char * argv[]) {
	...
	for(k=0; k <tilesPerRow; k++){;
		...
		// grey tiles
		#pragma omp parallel for private(grey) num_threads(nrOfThreads) 
		for (grey = 0; grey<(tilesPerRow);  grey++){
			if(grey != k){
			//printf("grey tile \n");
		...
		// white tiles
		#pragma omp parallel for private(whiteH, whiteV) num_threads(nrOfThreads)		
		for (whiteH = 0; whiteH<tilesPerRow; whiteH++ ){
			for(whiteV = 0; whiteV<tilesPerRow; whiteV++){
